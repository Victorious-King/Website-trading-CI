                  ...
                  <input type="hidden" name="last_link" value="<?php echo 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; ?>" />
                    ...
                    <div class="form-group">
                      ...
                      <div class="form-group">    
                        <div class="d-flex">
                            <div class="btcont">
                              <button class="btn" id="submit-button" type="submit"><?php echo $this->lang->line('post');?></button>
                            </div>  
...
<script type="text/javascript">
  ...
  jQuery(document).ready(function(){
    ...
    $("#fileInput").change(function(){
      ...
      if(img_len==imglimit){
        $(".photo_progress").hide();
        $("#multipleImages").show();return;
      }
        var total_file_tmp = $("#fileInput")[0].files.length;
        if(total_file_tmp==0) { $("#fileInput")[0].files = dt.files; return;}
        var list_tmp = new DataTransfer;
        var files_tmp = $("#fileInput")[0].files;
        img_len = dt.items.length;
        var diff = total_file_tmp+img_len-imglimit;
        if(diff>0){
        for(var i=0; i<total_file_tmp-diff; i++)
           list_tmp.items.add(files_tmp[i]) 

          $("#fileInput")[0].files = list_tmp.files;
          $("#multipleImages").show();
        }
        // console.log($("#fileInput")[0].files);
        ...
        for (var file_row of $("#fileInput")[0].files) {
          dt.items.add(file_row);
        }
        $("#fileInput")[0].files = dt.files;//final img number
        // console.log($("#fileInput")[0].files)
    ...
    $("#image_preview").on("click", ".delete_photo" , function(){
      ...
      for (var k = 0; k < $("#fileInput")[0].files.length; k++) {
      ...
      img_len = new_file.length;
      if(img_len < imglimit) $("#multipleImages").hide()
    })

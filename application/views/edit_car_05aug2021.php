...
<script>
  const imglimit= '<?php echo $_SESSION['img_limit']; ?>';
  var total_file;
  var scr;
  var u_id;
  var img_len;
  $(document).ready(function(){
    ...
    $("#fileInput").change(function(){
      ...
      if(img_len==imglimit){
        ...
        let total_file_tmp = $("#fileInput")[0].files.length;
        if(total_file_tmp==0) { $("#fileInput")[0].files = dt.files; return;}
        let list_tmp = new DataTransfer;
        let files_tmp = $("#fileInput")[0].files;
        img_len = dt.items.length;
        let diff = total_file_tmp+img_len-imglimit;
        console.log(img_len);
        if(diff>0){
        for(let i=0; i<total_file_tmp-diff; i++)
